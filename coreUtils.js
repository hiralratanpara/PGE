...
export default class coreUtils {
    ...
    async setText(element, text_to_be_set) {
        ...
        try {
            await t.typeText(element, text_to_be_set);
    ...
    async replaceText(element, text_to_be_set) {
        try {
            await t.typeText(element, text_to_be_set, { replace: true });
    ...
    async verifyTextContains(element, text_to_be_searched) {
        try {
            await t.expect(Selector(element).textContent).contains(text_to_be_searched)
    ...
    async hoverOnElement(element, text) {
        try {
            await t.hover(Selector(element).withText(text));
    ...
    async clickTextElement(element, text) {
        try {
            await t.click(Selector(element).withText(text));
    ...
    async selectText(element, text) {
        try {
            await t.click(Selector(element).filter(''))
    ...
    async clickAttributeElement(element, value) {
        try {
            await t.click(Selector(element).find('option').withAttribute('value', value))
    ...
    async verifyTextWithIndex(element, index) {
        try {
            await t.expect(Selector(element).nth(0).exists).ok()
    ...
    async clickIndexElement(element) {
        try {
            await t.click(Selector(element).nth(0))
    ...
    async clickIndexoneElement(element) {
        try {
            await t.click(Selector(element).nth(1))
            console.log('Clicked element ' + element + ' with index')
        }
        catch (err) {
            console.log('Can not click ' + element + ' with index')
